...
//  EssentialFeedTests
//
//  Created by Alexander Sundiev on 2023-01-08.
//
//
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    ...
                    default:
                        XCTFail("Expected retrieving twice from non empty cache to deliver same found reslut with feed \(feed) and timestamp \(timestamp), got \(firstResult) and \(secondResult) instead")
                    }
