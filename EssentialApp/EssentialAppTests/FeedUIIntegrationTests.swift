...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_feedImageView_rendersImageLoadedFromURL() {
        ...
        let imageData0 = anyImageData(.red, withAlpha: false)
        loader.completeImageLoading(with: imageData0, at: 0)
        XCTAssertEqual(view0?.renderedImage, imageData0, "Expected image for first view once first image loading completes successfully")
        XCTAssertEqual(view1?.renderedImage, .none, "Expected no image state change for second view once first image loading completes successfully")

        let imageData1 = anyImageData(.blue, withAlpha: false)
    ...
    private func anyImageData(_ color: UIColor = .red, withAlpha: Bool = true) -> Data {
        return UIImage.make(withColor: color).pngData(withAlpha: withAlpha)!
    }
}
